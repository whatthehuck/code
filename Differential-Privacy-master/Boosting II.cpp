...
public:
	db d0, d1;
	void setDelta(db d = 1.0) {
		delta = d;
	}
	void setEpsilon(db e = 1.0) {
		epsilon=e;
	}
	void calLambda() {
		...
		if(len >= MAXK) {
			...
			for(int i=0;i<MAXK;++i) 
				if(i<len) {
					nxt[x][i]=++las; ++ sz[x];
					if(!i) 
						d += buildTree(las, l+i, l+i+1); 
					else 
						buildTree(las, l+i, l+i+1);
					count[x] += count[nxt[x][i]];
					sum += z[nxt[x][i]];
			}else { 
				nxt[x][i] = -1;
...
int main(){
	...
	while(T --) {
		...
		DK.CopyData(A, 4);
		DK.setEpsilon(1); DK.setDelta( log(4) / log(MAXK+.0));
		DK.calLambda();
		DK.buildTree(1, 0, 4);
		DK._dfs(1, -1);
		//DK.show(1, 0, n);
